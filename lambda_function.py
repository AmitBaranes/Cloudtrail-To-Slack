...
SlackEmoji = ':warning:'
DisplayName = 'AWS-Cloudtrail-Alert'
AlertColor = '#FF8C00'
...
def get_changes(event):
    ...
    event_id = [val for key, val in event.get("detail").get("requestParameters").items() if 'Id' in key]
    if not event_id:
        get_event_id = event.get("detail").get("requestParameters").get("groupName")
        if not get_event_id:
            event_id = '-'
        else:
    ...
    return change_type, user_account, event_id, changes, timestamp, region


def send_slack_alert(change_type, user_account, event_id, changes, timestamp, region):
...
def lambda_handler(event, context):
    ...
    change_type, user_account, event_id, changes, timestamp, region = get_changes(event)
    send_slack_alert(change_type, user_account, event_id,changes, timestamp, region)
    return event